...
const generatePDF = async (name, email, event, date) => {
    ...
    doc.pipe(fs.createWriteStream(`certificates/${name}_${event}_completion_certificate.pdf`));
    ...
    doc
        ...
        .fontSize(10)
        .fill("#021c27")
        .text(`HR HEAD, XcitEducation
        `, startLine1, signatureHeight + 15, {
            columns: 1,
            columnGap: 0,
            height: 40,
            width: lineSize,
    ...
    // const generateQR = async text => {
    //     try {
    //         await QRCode.toFile(`qrCodes/${certiId}_completion.png`, text);
    //     } catch (err) {
    //         console.log(err);
    //     }
    // }
    // generateQR(`http://validate-your-certificate-xcitedu.com/${certiId}`);
    ...
    //     try {
    //         var base64data
    //         base64data = await QRCode.toDataURL(text);
    //         globalThis.data = base64data
    ...
    // QRCode.toDataURL(certiId)

    //  Mail
    const output = `
          <h2>Hi ${name}, Congratulations on completing the event in ${event}</h2>
        <p>You have successfully completed your event in ${event} with NGO. Please find your Certificate of completion below.</p>
        <h3>Your Details:</h3>
        <ul>
          <li>Name : ${name}</li>
          <li>Email : ${email}</li>
    ...
    let mailOptions = {
        ...
        subject: "Certificate Of Completion", // Subject line
        ...
        attachments: [
            {
                ...
                path: `certificates\\${name}_${event}_completion_certificate.pdf`
            },
        ],
    };
